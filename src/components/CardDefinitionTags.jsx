function CardDefinitionTags({ imageUrl, imageAlt, title, description, tags = [] }) {
  return (
    <div className="min-h-[530px] relative group max-w-sm rounded-xl overflow-hidden shadow-xl bg-black/30 backdrop-blur-lg transform transition-all duration-1000 ease-in-out hover:scale-[1.02] cursor-pointer">
      ...
      <img
        className="h-40 w-auto m-auto relative z-10 rounded-md mt-6"
        src={imageUrl}
        alt={imageAlt}
      />
      ...
      <div className="px-6 pt-4 pb-2">
        {tags.map((tag, index) => {
          if (typeof tag === "object" && tag !== null && tag.src) {
            return (
              <img
                key={index}
                src={tag.src}
                alt={tag.alt || ""}
                className="inline-block h-16 w-auto rounded-md mr-2 mb-2 align-middle"
                title={tag.alt || ""}
              />
            );
          }
